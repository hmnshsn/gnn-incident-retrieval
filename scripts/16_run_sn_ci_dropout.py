"""Train SN GNN with CI dropout and evaluate cold-start retrieval."""
...
def main() -> None:
    """Train CI-dropout GNN and save grouped retrieval metrics."""
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--ci-dropout-rate", type=float, default=0.2)
    parser.add_argument(
        "--ci-dropout-mode",
    ...
    logger = setup_logging("sn_exp08_ci_dropout")
    _set_seed(args.seed)
    ...
    logger.info("CI-dropout summary:\n%s", _format_summary(results))

    rate = f"{args.ci_dropout_rate:g}"
    output_path = Path(
        f"results/exp08_sn_ci_dropout/ci_dropout_"
        f"{args.ci_dropout_mode}_{rate}_{args.features}.json"
    )
